...
var asairlA_jj = new Array(
	...
	"1109 全国消防安全宣传教育日",
	"1110 世界青年节",
	"1111 双十一购物节 光棍节 国际科学与和平周(本日所属的一周)",
	"1112 孙中山诞辰纪念日",
	"1114 联合国糖尿病日",
	"1117 国际大学生节",
	"1121 世界问候日 世界电视日",
	...
	"1209 世界足球日",
	"1210 世界人权日",
	"1212 阿赛阿莲结婚纪念日 西安事变纪念日",
	"1213 南京大屠杀纪念日",
	"1220 澳门回归纪念",
	"1221 曹瑞莲生日1981 国际篮球日",
	"1224 平安夜",
	"1225 圣诞节",
	"1226 毛泽东诞辰纪念日",
	"1229 国际生物多样性日"
...
function sTerm(y, n) {
	log(1, "y=" + y + " n=" + n + " asairlA_rn[n]=" + asairlA_rn[n] + " Date.UTC(" + yearA + ",0,6,2,5)=" + Date.UTC(yearA, 0, 6, 2, 5) + " Date.UTC(" + yearB + ",0,1,0,0)=" + Date.UTC(yearB, 0, 1, 0, 0));
	//   var offDate=new Date( ( 31556925974.7*(y-yearA) + asairlA_rn[n]*60000  ) + Date.UTC(yearA,0,6,2,5) );
	var offDate = new Date((31556925974.7 * (y - yearA) + asairlA_rn[n] * 60000) - 2208549300000);
...
function calendar(y, m) {
	...
	for (var i = 0; i < this.length; i++) {
		...
		this[i] = new calElement(y, m + 1, i + 1, asairlA_sz[(i + this.firstWeek) % 7],
			lY, lM, lD++, lL,
	...
	log(1, "m: " + m + " tmp1: " + tmp1 + " " + asairlA_jq[m * 2] + " tmp2: " + tmp2 + " " + asairlA_jq[m * 2 + 1]);
	...
	log(1, "begin loop for(i in asairlA_jn)");
	for (i in asairlA_jn)
...
function drawCld(SY, SM) {
	...
	document.getElementById("gz").innerHTML = '　农历'
		+ cyclical(SY - yearA + 36) + '年【' + asairlA_sx[(SY - 4) % 12] + '】';
	...
	for (i = 0; i < 42; i++) {
		...
		if (sD > -1 && sD < cld.length) {
			sObj.innerHTML = sD + 1
			xObj.innerHTML = '<div class="asairlonk"><div class="asairlonc">' + cld[sD].lYear + ' 年 ' + cld[sD].sMonth
				+ ' 月 ' + cld[sD].sDay + ' 日<br />星期' + cld[sD].week + '<br />'
				+ '<span>农历' + (cld[sD].isLeap ? '闰 ' : ' ')
				+ cld[sD].lMonth + ' 月 ' + cld[sD].lDay + ' 日<br />'
				+ cld[sD].cYear + '年 ' + cld[sD].cMonth
				+ '月 ' + cld[sD].cDay + '日</span></div><div class="asairlont">' + cld[sD].asairlA_jqs + ' ' + cld[sD].solarFestival + ' ' + cld[sD].lunarFestival + '</div></div>';
			if (cld[sD].isToday) {
				//设置今天的背景色
...
function eestmshow() {
	...
	switch (d.getDay()) {
		...
		case 6: week = "<B>星期六</B>"; break;
		default: week = "<B>星期天</B>";
	}
...
document.write('<div id="AsaiTime" onClick="pushBtm(\'\')"></div>');

document.write('<div id="asaitiao">公历<span class="asaitiaox" onClick="pushBtm(\'YU\')">&lt;</span><select name="select" id="sy" onChange="changeCld()">');
for (var yri = yearA; yri < yearB + 1; yri++) { document.write("<option>" + yri + "</option>"); }
document.write('</select><span class="asaitiaox" onClick="pushBtm(\'YD\')">&gt;</span>年<span class="asaitiaox" onClick="pushBtm(\'MU\')">&lt;</span><select name="select2" id="sm" onChange="changeCld()">');
for (var mhi = 1; mhi < 13; mhi++) { document.write("<option>" + mhi + "</option>"); }
document.write('</select><span class="asaitiaox" onClick="pushBtm(\'MD\')">&gt;</span>月<span id="gz"></span></div>');
